...
func RoomConn(c *websocket.Conn, p *Peers) {
	...
	if os.Getenv("ENVIRONMENT") == "PRODUCTION" {
		...
		config = webrtc.Configuration{
			...
			ICEServers: []webrtc.ICEServer{
				{
					// URLs: []string{"stun:146.196.106.202:8010"},
					URLs: []string{"stun:stun.relay.metered.ca:80"},
				},
				{
					URLs: []string{
						// "turn:146.196.106.202:8010",
	...
	for _, typ := range []webrtc.RTPCodecType{webrtc.RTPCodecTypeVideo, webrtc.RTPCodecTypeAudio} {
		fmt.Println("enter to For loop RTPCodecTypeVideo")
	...
	peerConnection.OnICECandidate(func(i *webrtc.ICECandidate) {
		if i == nil {
			fmt.Println("eh kosong bs")
			return
		}
		fmt.Println("masuk candidate")
	...
	for {
		...
		case "candidate":
			fmt.Println("ada yang ngirim nih")
			...
			// candidateString1 := []byte(message.Data)

			// if writeErr := newPeer.Websocket.WriteJSON(&websocketMessage{
			// 	Event: "candidate",
			// 	Data:  string(candidateString1),
			// }); writeErr != nil {
			// 	log.Println(writeErr)
			// }
		case "answer":
